...
const COQUETAS = [
  ...
  "Tu dedo tiene buen gusto 👀",
  "Si tocas otra vez, me haces sonreír 🙈",
  "Plot twist: esto era para ti ✨",
  "Esto se está poniendo peligroso… (para mi concentración) 😏",
  "Toca donde quieras… el fondo reacciona por ti 🎨",
  "No sé qué haces, pero se ve lindo. Sigue 😌",
  "Si alguien te dijo feliz cumple… yo traje algo distinto 🎁",
  "Esto es un mini secreto. No le digas a nadie 🤫"
];
...
// Desbloqueo final
function unlock(){
  // Evitar múltiples unlocks
  if (!final.hidden) return;

  setPill("Ok… esto ya se puso serio 😌");
  // Confetti-ish: bursts grandes
  for (let i=0;i<6;i++){
    setTimeout(() => spawnBurst(rand(80, w-80), rand(120, h-220), 1.6), i*120);
  }

  // Mostrar final después de un “beat”
  setTimeout(() => {
    final.hidden = false;
  }, 520);
}
