...
// Overlay cumple (DEBE existir en el HTML)
const bdayOverlay = document.getElementById("bdayOverlay");
if (bdayOverlay) bdayOverlay.hidden = true;
...
function updateLock() {
  ...
  lockIcon.textContent = (secretCount >= SECRET_TARGET) ? "🔓" : "🔒";
}
...
function addSecret(n = 1) {
  ...
  updateLock();
  if (secretCount >= SECRET_TARGET) unlock();
}

// Overlay cumple
function showBirthday() {
  if (!bdayOverlay) return;

  bdayShowing = true;
  bdayOverlay.hidden = false;

  // Ya no se cierra automáticamente
  // Ya no se cierra al tocar
}


function hideBirthday() {
  if (!bdayOverlay || bdayOverlay.hidden) return;

  bdayOverlay.classList.add("hide");
  setTimeout(() => {
    bdayOverlay.classList.remove("hide");
    bdayOverlay.hidden = true;
    bdayShowing = false;
    setPill("Sigue tocando… aún reacciona ✨");
  }, 320);
}

// Unlock: ahora muestra el “Feliz cumpleaños” en pantalla
function unlock() {
  if (secretUnlocked) return;
  secretUnlocked = true;

  setPill("Ok… encontraste el secreto 🤍");

  sparkleRain(900);

  for (let i = 0; i < 4; i++) {
    setTimeout(() => spawnBurst(rand(80, w - 80), rand(120, h - 260), 1.1), i * 140);
  }
...
function maybeBubble(x, y) {
  ...
  if (secretCount <= 2) setPill(pick(IT_JOKE));
  else setPill("Ok… esto ya no parece IT 😅");

  if (Math.random() < 0.80) showBubble(pick(COQUETAS), x, y);
}
